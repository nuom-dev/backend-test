...
const createBooking = async (req, res) => {
  try {
    ...
    Booking.find({
      ...
      if (booking.length == 1) {
        const error = "Sorry, You have already registered for this course!"
                    ...
                    if (location) {
                      ...
                      await Trainer.find({
                        ...
                        await Booking.create({
                          ...
                          .then((booking) => {
                            if (booking) {
                              const msg = `Booking successfully created!`;
                              return res.status(200).json(responseMessage(booking, msg, true ));
                            }
                          })
                          .catch((err) => {
                            const error = `An error occured while creating booking: ${err}`
